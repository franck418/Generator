...
<html>
  <head>
	...
	<script type="text/javascript" src="${basePath}static/common/js/binfTips.js"></script>
		...
		<tr>
			...
			<td width="35%">
				<input class="inputsearch" type="text" id="name" name="name"
					value="${ddd}" size="15" maxlength="15" />
			</td>
			<td width="15%" style="text-align: right;">
				状态：
			</td>
...
<script type="text/javascript">
	function toAdd(){
		...
		binf.show(_url,'新增分类',450,250);
	}
	
	function toEdit(id){
		var _url = '${basePath}sys/$!{lowerName}/toedit.do?id='+id;
		binf.show(_url,'编辑分类信息',450,250);
	}
	...
	function deletes(){
		...
		binf.submit(theform,function callback(data){
